...
class Ns_Followers:
    def __init__(self, userid, username, session):
...
class TelegramBotHandler:
    def __init__(self, bot_token):
        ...
        def send_welcome(message):
            self.bot.reply_to(message, "Welcome! Please enter your Instagram username:")

        @self.bot.message_handler(func=lambda message: message.chat.id not in self.users_data)
        def get_username(message):
            self.users_data[message.chat.id] = {'username': message.text}
            self.bot.reply_to(message, "Got it! Now please enter your Instagram user ID:")

        @self.bot.message_handler(func=lambda message: 'userid' not in self.users_data[message.chat.id])
        def get_userid(message):
            self.users_data[message.chat.id]['userid'] = message.text
            self.bot.reply_to(message, "Thanks! Finally, please enter your Instagram session ID:")

        @self.bot.message_handler(func=lambda message: 'session' not in self.users_data[message.chat.id])
        def get_session(message):
            self.users_data[message.chat.id]['session'] = message.text
            user_data = self.users_data[message.chat.id]
            self.bot.reply_to(message, "Thank you! Now attempting to log in...")
            self.start_instagram_process(user_data['userid'], user_data['username'], user_data['session'])

    def start_instagram_process(self, userid, username, session):
        cl = Ns_Followers(userid, username, session)
        try:
            login_response = cl.Login_v61()
            self.bot.send_message(chat_id, f"Login_v61 response: {login_response}")
            login_response = cl.Login_v62()
            self.bot.send_message(chat_id, f"Login_v62 response: {login_response}")
            while True:
                suff = cl.Get_Suggest()
                if "suggests" in suff.text:
                    for items in suff.json()['suggests']:
                        order_response = cl.PlaceOrder(items)
                        self.bot.send_message(chat_id, f"Order placed: {order_response}")
                        time.sleep(random.randint(10, 15))
                else:
                    self.bot.send_message(chat_id, f"Suggest response: {suff.text}")
                    time.sleep(10)
        except Exception as E:
...
if __name__ == '__main__':
    sanchit=TelegramBotHandler("7165271571:AAEPYjwpO0WJUXUg2Tj9XYZ0DGTNLGt5bDU")
    sanchit.start()
